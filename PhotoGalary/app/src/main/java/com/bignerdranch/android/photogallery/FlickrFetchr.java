...
//    }
    ...
    private void parseItems0(List<GalleryItem> items, JSONObject jsonBody) {
        try {
            ...
            List<GalleryItem> collection = new Gson().fromJson(jsonBody.getJSONObject("photos").getString("photo"), listType);
            List<GalleryItem> withNullLinks = new ArrayList<>();
//            for (GalleryItem item : collection) {
//                if (item.getUrl() == null || item.getOwner() == null) {
//                    withNullLinks.add(item);
//                }
//            }
            items.addAll(collection);
            items.removeAll(withNullLinks);
        } catch (JSONException je) {
